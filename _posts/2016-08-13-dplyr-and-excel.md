...
在 R 中要產生上面的樞紐分析表，其語法如下。可看到我們先在 `group_by()` 加入 Embarked 變數，做完 `summarise()` 後，設定 `spread()` 的 key 為 Embarked 即可完成。但此方法只能產生一層的欄標籤，無法做到兩層以上的欄標籤。
...
此外，`filter()` 也能作為列標籤或欄標籤的篩選。如下面的樞紐分析表，我們僅篩選 Pclass 是 2 跟 3。在 R 中可以透過下面的語法完成。可看到 `filter()` 當中設定 `Pclass != 1`。
...
必須注意一點 R 進行 `summarize()` 運算後，原本資料集的變數就會只剩有在 `group_by()` 內的變數。因此，若想要做樞紐分析表的報表篩選，務必將 `filter()` 放在 `summarize()` 之前。舉例來說，如下面的 R 語法就會回報錯誤，因為 Embarked 變數已不在 `summarize()` 後的資料集中。
